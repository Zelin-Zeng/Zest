...
namespace Zest { namespace Lib { namespace ThreadPool {
...
public:
	...
	{
		std::promise<std::result_of_t<TFunc(TArgs...)>> promise;
	...
	{
		std::promise<std::result_of_t<TFunc()>> promise;
	...
	void AddThread() noexcept
	{
		...
		std::shared_ptr<ThreadPool> spThis{ this };
		auto thread = std::make_unique<Thread>([spThis, command]() noexcept
		{
			for (;;)
			{
				// Exit loop
				...
				if (spThis->m_taskQueue->FrontAndPop(spFunc))
				{
					(*spFunc)();
				}

				std::unique_lock<std::mutex> lock(spThis->m_mutex);
				spThis->m_taskQueueChangeSignalBus.wait(lock, [spThis]() {
					return !spThis->m_taskQueue->IsEmpty() || spThis->m_isStop || spThis->m_isDone;
				});
			}
		});
